...
namespace TuiFly.FlySearchApi.Api.Controllers.Fligths.Validations
{
    ...
    public class FlightRequestValidator : AbstractValidator<GetFlightsRequest>
    {
        public FlightRequestValidator()
        {
            ...
            RuleFor(m => m)
                .Must(m => (m.DepartureDate.Date.CompareTo(m.ReturnDate.Date) == 0 || m.DepartureDate.Date.CompareTo(m.ReturnDate.Date) < 0) && m.DepartureDate.Date.CompareTo(DateTime.Now.AddDays(-1)) > 0)
